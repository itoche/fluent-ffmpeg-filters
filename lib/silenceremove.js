...
class SilenceremoveFilter {
  ...
  constructor (ffmpeg) {
    this.ffmpeg = ffmpeg;
    SilenceremoveFilter.prototype.withStart_periods = start_periods;
    SilenceremoveFilter.prototype.withStart_duration = start_duration;
    SilenceremoveFilter.prototype.withStart_threshold = start_threshold;
    SilenceremoveFilter.prototype.withStop_periods = stop_periods;
    SilenceremoveFilter.prototype.withStop_duration = stop_duration;
    SilenceremoveFilter.prototype.withStop_threshold = stop_threshold;
    SilenceremoveFilter.prototype.withLeave_silence = leave_silence;
    SilenceremoveFilter.prototype.withDetection = detection;
    SilenceremoveFilter.prototype.withWindow = window;
  }
  ...
  build() {
    let opt = {};
    if (this.start_periods) {
      opt['start_periods'] = this.start_periods;
    }
    if (this.start_duration) {
      opt['start_duration'] = this.start_duration;
    }
    if (this.start_threshold) {
      opt['start_threshold'] = this.start_threshold;
    }
    if (this.stop_periods) {
      opt['stop_periods'] = this.stop_periods;
    }
    if (this.stop_duration) {
      opt['stop_duration'] = this.stop_duration;
    }
    if (this.stop_threshold) {
      opt['stop_threshold'] = this.stop_threshold;
    }
    if (this.leave_silence) {
      opt['leave_silence'] = this.leave_silence;
    }
    if (this.detection) {
