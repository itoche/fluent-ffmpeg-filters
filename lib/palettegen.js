...
class PalettegenFilter {
  ...
  build() {
    let opt = {};
    if (this._max_colors) {
      opt['max_colors'] = this._max_colors;
    }
    if (this._reserve_transparent) {
      opt['reserve_transparent'] = this._reserve_transparent;
    }
    if (this._stats_mode) {
      opt['stats_mode'] = this._stats_mode;
    }
