...
class AsetnsamplesFilter {
  ...
  constructor (ffmpeg) {
    this.ffmpeg = ffmpeg;
    AsetnsamplesFilter.prototype.withNb_out_samples = this.nb_out_samples;
    AsetnsamplesFilter.prototype.withPad = this.pad;
  }
  ...
  build() {
    let opt = {};
    if (this._nb_out_samples) {
      opt['nb_out_samples'] = this._nb_out_samples;
    }
    if (this._pad) {
      opt['pad'] = this._pad;
    }
