...
class CompandFilter {
  ...
  constructor (ffmpeg) {
    this.ffmpeg = ffmpeg;
    CompandFilter.prototype.withDecays = decays;
    CompandFilter.prototype.withPoints = points;
    CompandFilter.prototype.withSoft_knee = soft_knee;
    CompandFilter.prototype.withGain = gain;
    CompandFilter.prototype.withVolume = volume;
    CompandFilter.prototype.withDelay = delay;
  }
  ...
  build() {
    ...
    if (this.points) {
      opt['points'] = this.points;
    }
    if (this.soft_knee) {
      opt['soft-knee'] = this.soft_knee;
    }
    if (this.gain) {
      opt['gain'] = this.gain;
    }
