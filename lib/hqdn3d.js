...
class Hqdn3dFilter {
  ...
  constructor (ffmpeg) {
    this.ffmpeg = ffmpeg;
    Hqdn3dFilter.prototype.withLuma_spatial = luma_spatial;
    Hqdn3dFilter.prototype.withChroma_spatial = chroma_spatial;
    Hqdn3dFilter.prototype.withLuma_tmp = luma_tmp;
    Hqdn3dFilter.prototype.withChroma_tmp = chroma_tmp;
  }
  ...
  build() {
    let opt = {};
    if (this.luma_spatial) {
      opt['luma_spatial'] = this.luma_spatial;
    }
    if (this.chroma_spatial) {
      opt['chroma_spatial'] = this.chroma_spatial;
    }
    if (this.luma_tmp) {
      opt['luma_tmp'] = this.luma_tmp;
    }
    if (this.chroma_tmp) {
      opt['chroma_tmp'] = this.chroma_tmp;
    }
